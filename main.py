...
'''
 ...
 for item in list:
    if "ש".decode("utf-8","ignore") in item.text:
             print item.text
...
location=-1
hitStock=-1
# stocks = [u"בזק",u"פועלים"]
stock_ids = []

stocks_file=open('/home/dan/Downloads/stock_id','r')
for line in stocks_file:
    if line != '':
        stock_ids.append(line.replace("\n",""))
#    print line
...
for i in range(0,2500):
   ...
   for item in list:
       if item.text in stock_ids:
                    try:
                        site2=urllib.urlopen("http://www.bizportal.co.il/Quote/Stock/GeneralView/"+item.text)
                    except:
                        break
                    stockSoup = BeautifulSoup(site2,'html5lib')
                    stockList = stockSoup.find_all("div")
                    for stockItem in stockList:
                        # if item.text in stockItem.text:
                                if stockItem.attrs.has_key('class'):
                                        if "titleRight" in stockItem['class']:
                                                ...
                                                inputString="insert into stocks values ("+item.text+",NULL,"+value+")"
                                                print ("stock "+item.text+" added with price: "+value)
                                                # print inputString
